...
class TUTORIAL_OT_3DTextMaker(bpy.types.Operator):
    ...
    def execute(self, context):
        for i, c in enumerate(self.text):
            makesplittext(i, c, self.fontpath, self.extrude,
                          self.offset, self.bevel_depth, self.bevel_resolution)
...
class TUTORIAL_OT_SaveText(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for i, c in enumerate(bpy.context.scene.text):
            ...
            if self.save_option_full_text is True:
                savetext2(i, bpy.context.scene.text, self.dirname)
            else:
